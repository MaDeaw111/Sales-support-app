...
CREATE TABLE shipment_containers (
  ...
  container_type TEXT NOT NULL CHECK(container_type IN ('20GP','40GP','40HC','OTHER')),
  seal_no TEXT,
  loading_date TEXT,
...
CREATE TABLE customer_credits (
  ...
  customer_id TEXT NOT NULL REFERENCES customers(customer_id),
  credit_no TEXT NOT NULL CHECK(trim(credit_no) <> ''),
  credit_date TEXT,
  currency TEXT NOT NULL CHECK(currency IN ('USD','THB','EUR')),
  original_amount REAL NOT NULL CHECK(original_amount > 0),
  status TEXT NOT NULL DEFAULT 'OPEN' CHECK(status IN ('OPEN','PARTIALLY_USED','FULLY_USED','CANCELLED')),
  note TEXT,
  created_by TEXT NOT NULL REFERENCES users(user_id),
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_by TEXT REFERENCES users(user_id),
  updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(customer_id, credit_no)
);
...
CREATE TABLE shipment_audit_events (
  ...
  event_type TEXT NOT NULL CHECK(event_type IN ('DI_CREATED','DI_UPDATED','DI_CONFIRMED','DI_CANCELLED','SHIPMENT_CREATED','SHIPMENT_UPDATED','STATUS_CHANGED','BOOKING_RECORDED','CONTAINER_ADDED','CONTAINER_UPDATED','INVOICE_ADDED','INVOICE_UPDATED','CREDIT_APPLIED','DOCUMENTS_SENT','SHIPMENT_CANCELLED')),
  actor_id TEXT NOT NULL REFERENCES users(user_id),
  metadata_json TEXT,
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
