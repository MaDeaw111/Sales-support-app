...
export function createCustomerHandler({ repo, resolveUser }) {
  return async function handle(request, env) {
    ...
    const isAllowedMutate = ['ADMIN', 'MANAGER', 'SALES_SUPPORT', 'EXTERNAL_SALES'].includes(user.role);
    const isFullAccess = ['ADMIN', 'MANAGER', 'SALES_SUPPORT'].includes(user.role);
    ...
    if (path === '/api/customers' && request.method === 'GET') {
      ...
      } else if (user.role === 'EXTERNAL_SALES') {
        customers = await repo.listCustomers({ ownerUserId: user.user_id });
      }
      return json({ status: 'SUCCESS', data: customers });
    }
    
    if (path === '/api/customers' && request.method === 'POST') {
      if (!isAllowedMutate || ['EXPORT', 'PRODUCTION_WAREHOUSE'].includes(user.role)) {
    ...
    if (match) {
      ...
      if (request.method === 'GET') {
        ...
        if (!isFullAccess) {
          if (user.role === 'EXTERNAL_SALES' && customer.ownerId === user.user_id) {
            // allowed
          } else {
            return json({ status: 'ERROR', message: 'Customer not found.' }, 404);
          }
      ...
      if (request.method === 'PUT') {
        ...
        if (user.role === 'EXTERNAL_SALES') {
          if (customer.ownerId !== user.user_id) {
            return json({ status: 'ERROR', message: 'Customer not found.' }, 404);
          }
