...
test('POST /api/shipments/:id/expenses: validation and retrieval', async () => {
  ...
  const reqUnauth = makeRequest('/api/shipments/SH1/expenses', 'POST', payload);
  ...
  const reqMgr = makeRequest('/api/shipments/SH1/expenses', 'POST', payload, 'token-mgr');
  ...
  assert.equal(dataMgr.data.expense.amount, 10000);

  // 3. GET lists expenses and returns total
  const reqGet = makeRequest('/api/shipments/SH1/expenses', 'GET', null, 'token-mgr');
  const resGet = await worker.fetch(reqGet, { DB: wrappedDb });
  assert.equal(resGet.status, 200);
  const dataGet = await resGet.json();
  assert.equal(dataGet.data.expenses.length, 1);
  assert.equal(dataGet.data.totalActualExportCostThb, 10000);
});
